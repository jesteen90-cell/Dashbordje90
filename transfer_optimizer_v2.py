...
def best_xi(squad,gw):
 ...
 cap=max(best[1],key=lambda p:float(p['_x'].get(gw,0)));return {'raw':best[0],'captain':cap,'xi':best[1]}
...
def _incoming_pools(players,squad,gws,weights,per_pos=12):
 ...
 for pos in POS_COUNTS:
  xs=[p for p in players if int(p['element_type'])==pos and int(p['id']) not in owned];xs.sort(key=lambda p:sum(float(p['_x'].get(g,0))*weights.get(g,1) for g in gws),reverse=True);pools[pos]=xs[:per_pos]
 return pools
def _one_transfer_states(st,players,pools,gw):
 ...
 for sell in st['squad']:
  ...
  for buy in pools[int(sell['element_type'])]:
   ...
   if legal(ns):out.append((ns,budget-int(buy['now_cost']),[(int(sell['id']),int(buy['id']))]))
 return out
def optimize(players,squad,bank,gws,weights,free_transfers=1,beam_width=70,per_pos=12,save_ft_value=.45,max_saved_ft=5,hit_cost=4.,max_transfers_per_gw=2):
 pools=_incoming_pools(players,squad,gws,weights,per_pos);beam=[{'squad':list(squad),'bank':int(bank),'ft':max(1,min(max_saved_ft,int(free_transfers))),'score':0.,'moves':[],'hits':0}]
 for gw in gws:
  nxt=[]
  for st in beam:
   nft=min(max_saved_ft,st['ft']+1);val=gw_value(st['squad'],gw)*weights.get(gw,1)+save_ft_value*nft;nxt.append({**st,'ft':nft,'score':st['score']+val,'moves':st['moves']+[{'gw':gw,'action':'bank','transfers':0,'hit':0}]})
   first=_one_transfer_states(st,players,pools,gw)
   for ns,nb,pairs in first:
    k=1;hit=max(0,k-st['ft'])*hit_cost;nft=min(max_saved_ft,max(0,st['ft']-k)+1);val=gw_value(ns,gw)*weights.get(gw,1)-hit+save_ft_value*nft;nxt.append({'squad':ns,'bank':nb,'ft':nft,'score':st['score']+val,'hits':st['hits']+int(hit),'moves':st['moves']+[{'gw':gw,'action':'transfer','pairs':pairs,'transfers':1,'hit':int(hit)}]})
 ...
 best=beam[0];baseline=sum(gw_value(squad,g)*weights.get(g,1) for g in gws);return {'score':best['score'],'baseline_score':baseline,'gain':best['score']-baseline,'bank':best['bank'],'free_transfers':best['ft'],'hit_points':best['hits'],'moves':best['moves'],'squad':best['squad']}
