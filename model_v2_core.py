...
Inputs must be pre-deadline features only. Model 3.8 adds 2026/27-aware bonus
priors so old-season bonus history does not dominate after the BPS rebalance.
"""
...
def bonus_prior_2627(position):
 # 2026/27 BPS reduces CBI overlap and removes the being-tackled penalty,
 # shifting a little bonus expectation from centre-back profiles toward GK,
 # attacking full-backs and attacking players. Keep the adjustment deliberately
 # small: observed current-season bonus quickly takes over as minutes accrue.
 return {1:.29,2:.26,3:.41,4:.43}.get(int(position),.35)
