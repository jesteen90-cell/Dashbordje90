...
Inputs must be pre-deadline features only. Model 4.1 makes goalkeeper save
expectation fixture-aware while preserving the discrete every-three-saves rule.
"""
...
def goalkeeper_save_multiplier(opponent_goal_lambda,baseline=1.35):
 """Bounded fixture adjustment for save volume.

    Opponent scoring threat is an imperfect proxy for shots on target, so only
    part of the xG signal is passed through. This prevents double-counting the
    same fixture strength already used for clean sheets/conceded points.
    """
...
def project(inp):
 ...
 bonus_prior=bonus_prior_2627(pos);bonus90=beta_shrink(float(inp.get('bonus90',0)),hist,bonus_prior,720);bonus=min(1.8,bonus90*frac)*avail;yellow=-clamp(float(inp.get('yellow90',0))*frac,0,.5);red=-3*clamp(float(inp.get('red90',0))*frac,0,.08);total=max(0,md['appearance_pts']+goals+assists+cs+conceded+saves+dc+bonus+yellow+red);u=uncertainty(total,md,pos,g90,a90,cs_prob,frac,save90*save_mult,dc_prob,bonus);return {'total':total,'xmins':md['xmins'],'p_start':md['p_start'],'p_60':md['p_60'],'cs_probability':cs_prob,'defensive_exposure':round(clamp(md.get('avg_start_mins',90)/90,.67,1.0),4),'appearance':md['appearance_pts'],'goals':goals,'assists':assists,'clean_sheet':cs,'conceded':conceded,'save_fixture_multiplier':save_mult,'expected_saves':expected_saves,'saves':saves,'defensive':dc,'defcon_probability':dc_prob,'defcon90_used':dc90,'bonus':bonus,'bonus90_used':bonus90,'bonus_prior_used':bonus_prior,'cards':yellow+red,'goal90_used':g90,'assist90_used':a90,'goal_prior_used':gprior,'assist_prior_used':aprior,'attack_evidence_minutes':attack_mins,**u}
