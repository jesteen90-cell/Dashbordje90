"""FPL Model v2.4 shared core.

Component expected-points engine for live projection and walk-forward tests.
Inputs must be pre-deadline features only. The engine now returns both mean xP
and an uncertainty estimate so decision logic can distinguish stable and
high-variance projections with the same expected value.
"""
...
def uncertainty(mean,md,pos,g_rate,a_rate,lam,frac,save90,dc_prob,bonus):
    """Approximate predictive variance from scoring-event uncertainty.

    This is intentionally conservative and transparent rather than pretending
    independence is exact. A covariance inflation term absorbs shared match
    state (minutes, team attack, clean sheet and bonus correlations).
    """
    # Appearance: 0/1/2-point mixture approximated from p_app and p60.
    papp=md['p_start']+md['p_sub']; eapp=md['appearance_pts']
    eapp2=max(0,papp-md['p_60'])*1 + md['p_60']*4
    v_app=max(0,eapp2-eapp*eapp)
    # Poisson event counts: Var(points*N)=points^2*lambda.
    vg=(GOAL_PTS[pos]**2)*max(0,g_rate*frac)
    va=9*max(0,a_rate*frac)
    # Clean sheet is a Bernoulli event conditional on 60+ minutes.
    pcs=md['p_60']*math.exp(-lam);vcs=(CS_PTS[pos]**2)*pcs*(1-pcs)
    # Save counts are noisy; approximate underlying saves as Poisson then /3 scoring.
    vs=(max(0,save90)*frac)/9 if pos==1 else 0
    vdc=4*dc_prob*(1-dc_prob) if pos in (2,3,4) else 0
    # Bonus 0..3 has substantial residual variance not captured by mean.
    vb=max(.10,bonus*(1.35-bonus/3)) if bonus>0 else .08
    raw=v_app+vg+va+vcs+vs+vdc+vb
    # Cards/conceded/BPS correlations and model uncertainty inflation.
    variance=max(.35,raw*1.18 + .12*mean*mean)
...
def project(inp):
    ...
    frac=md['xmins']/90;hist=float(inp.get('minutes_history',0));atk=float(inp.get('attack_multiplier',1))
