...
Inputs must be pre-deadline features only. Model 4.0 improves goalkeeper save
points by modelling the discrete every-three-saves rule rather than save/3.
"""
...
def expected_save_points(mean_saves):
 """Expected FPL points from the discrete 1 point per three saves rule."""
 lam=max(0.0,float(mean_saves or 0));
 if lam<=0:return 0.0
 # E[floor(S/3)] for S~Poisson(lam); tail above 30 saves is negligible.
