...
Inputs must be pre-deadline features only. Model 3.9 adds overdispersed match-level
DEFCON threshold probabilities so season-average action rates are not treated as
Poisson-stable from match to match.
"""
...
def negbin_pmf(k,mean,dispersion):
 ...
 mean=max(.0001,float(mean));r=max(.25,float(dispersion));p=r/(r+mean)
...
def defcon_threshold_probability(mean,threshold,position):
 # Defensive actions are clustered by game state/opponent. A Poisson model is
 # too certain around the 10/12-action cut-off, so use mild overdispersion.
 # DEF gets slightly lower dispersion than MID/FWD because CBIT roles are more
 # stable; MID/FWD recoveries are more game-state dependent.
 r={2:8.0,3:6.0,4:5.0}.get(int(position),6.0);m=max(.0001,float(mean));t=max(1,int(threshold))
 return clamp(1-sum(negbin_pmf(k,m,r) for k in range(t)))
...
def project(inp):
 ...
 bonus_prior=bonus_prior_2627(pos);bonus90=beta_shrink(float(inp.get('bonus90',0)),hist,bonus_prior,720);bonus=min(1.8,bonus90*frac)*avail;yellow=-clamp(float(inp.get('yellow90',0))*frac,0,.5);red=-3*clamp(float(inp.get('red90',0))*frac,0,.08);total=max(0,md['appearance_pts']+goals+assists+cs+conceded+saves+dc+bonus+yellow+red);u=uncertainty(total,md,pos,g90,a90,cs_prob,frac,save90,dc_prob,bonus);return {'total':total,'xmins':md['xmins'],'p_start':md['p_start'],'p_60':md['p_60'],'cs_probability':cs_prob,'defensive_exposure':round(clamp(md.get('avg_start_mins',90)/90,.67,1.0),4),'appearance':md['appearance_pts'],'goals':goals,'assists':assists,'clean_sheet':cs,'conceded':conceded,'saves':saves,'defensive':dc,'defcon_probability':dc_prob,'defcon90_used':dc90,'bonus':bonus,'bonus90_used':bonus90,'bonus_prior_used':bonus_prior,'cards':yellow+red,'goal90_used':g90,'assist90_used':a90,'goal_prior_used':gprior,'assist_prior_used':aprior,'attack_evidence_minutes':attack_mins,**u}
