...
fg=d.get('final_transfer_gate') or {}; prod=d.get('decision_layer') or {}; package=(d.get('action_package_selection') or {}).get('selected') or {}
selected_index=package.get('candidate_index',(d.get('candidate_selection') or {}).get('selected_candidate_index'))
...
score=float(conf.get('score') or package.get('reliability') or 0)
make=float(reg.get('regret_make_transfer') or .5); save=float(reg.get('regret_save_ft') or .5)
label=nec.get('label')
info=float(tim.get('information_value') or (float(package.get('information_penalty') or 0)/.55 if package.get('information_penalty') is not None else 0))
lock=float(tim.get('lock_risk') or 0)
blockers=[]; warnings=[]
if package.get('kind')=='bank': blockers.append('Felles handlingssammenligning velger å spare gratisbyttet.')
...
d['headline']=headline
d['deadline_lock']={'version':'1.1-package-aware','affects_transfer_ranking':False,'production_approved':approved,'verdict':verdict,'headline':headline,'selected_action_kind':package.get('kind'),'selected_action_label':package.get('label'),'selected_action_hit':package.get('hit',0),'confidence_score':round(score,2),'necessity':label,'regret_make_transfer':round(make,2),'regret_save_ft':round(save,2),'information_value':round(info,2),'budget_lock_risk':round(lock,2),'blockers':blockers,'warnings':warnings,'rule':'Deadline Lock evaluates the one selected action and may downgrade it, never promote a production-rejected transfer.'}
