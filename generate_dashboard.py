...
DEFAULT_WEIGHTS=[1,.9,.8,.7,.62,.55]; POS={1:'GK',2:'DEF',3:'MID',4:'FWD'}

def get(path,optional=False):
 try:
  r=requests.get(f"{BASE}/{path.lstrip('/')}",headers={'Accept':'application/json','User-Agent':'fpl-autopilot-model-v2.3'},timeout=30);r.raise_for_status();return r.json()
 except Exception:
...
def strategy_weights():
 ...
 if p.exists():
  try:
   x=json.loads(p.read_text());w=[float(v) for v in x.get('weights',[])]
   if len(w)>=3:return w[:6]
  except Exception:pass
 return DEFAULT_WEIGHTS

boot=get('bootstrap-static/');fixtures=get('fixtures/');players=boot['elements'];events=boot['events'];teams={int(t['id']):t['name'] for t in boot['teams']};byid={int(p['id']):p for p in players}
...
TARGET=int(event['id']);deadline=event['deadline_time'];W=strategy_weights();GWS=list(range(TARGET,min(38,TARGET+len(W)-1)+1));weights={g:W[i] for i,g in enumerate(GWS)}
fm={g:{} for g in GWS}
for f in fixtures:
 g=f.get('event')
...
def core_input(p,f):
 ...
 attack={1:1.22,2:1.11,3:1,4:.89,5:.78}[f['fdr']]*(1.035 if f['home'] else .97);opp_lambda={1:.72,2:1.00,3:1.32,4:1.70,5:2.15}[f['fdr']]*(.93 if f['home'] else 1.07)
...
def project(p,g):
 cs=[core_project(core_input(p,f)) for f in fm.get(g,{}).get(int(p['team']),[])]
 keys=('total','xmins','appearance','goals','assists','clean_sheet','saves','defensive','bonus','conceded','cards','cs_probability')
 return {k:sum(c.get(k,0) for c in cs) for k in keys} if cs else {k:0 for k in keys}
...
pool=[p for p in players if availability(p)>=.5 or int(p['id']) in {int(x['id']) for x in squad}]
opt=optimize_transfers(pool,squad,bank,GWS,weights,free_transfers=free_transfers,beam_width=70,per_pos=12,save_ft_value=.45,hit_cost=4,max_transfers_per_gw=2)
plan=opt['moves'];first=plan[0] if plan else {'gw':TARGET,'action':'bank'};first_pairs=first.get('pairs',[]) if first.get('action')=='transfer' else [];first_out=byid[first_pairs[0][0]] if first_pairs else None;first_in=byid[first_pairs[0][1]] if first_pairs else None;after=apply_move(squad,first);cur=lineup(squad,TARGET);aft=lineup(after,TARGET);gain=float(opt.get('gain',0));go=first.get('action')=='transfer' and gain>1
...
outs={int(a) for a,b in first_pairs};ins={int(b) for a,b in first_pairs}
future=[];sim=list(squad)
for g in GWS:
 m=next((x for x in plan if x.get('gw')==g),None);sim=apply_move(sim,m);o=lineup(sim,g);future.append({'gw':g,'captain':o['captain']['web_name'],'captain_xp':round(o['captain']['_x'][g],2),'xi_xp':round(o['raw'],2),'action':public_move(m)})
# transparent one-transfer alternatives
...
cands.sort(key=lambda x:x['horizon_gain'],reverse=True);cands=cands[:10]
headline='GJØR BYTTET' if go else ('SPAR BYTTET' if first.get('action')=='bank' else 'VENT / BANK');summary='Modell v2.3 planlegger flere gameweeks samtidig, med korrekte gratisbytter, mulighet for to bytter og eksplisitt -4-kostnad for hits.'
data={'model_version':'2.3-strategy','generated_at':datetime.now(timezone.utc).isoformat().replace('+00:00','Z'),'gw':TARGET,'deadline_time':deadline,'headline':headline,'summary':summary,'source_snapshot_gw':snapshot_gw,'free_transfers_assumed':free_transfers,
'optimizer':{'horizon_gws':GWS,'weights':[weights[g] for g in GWS],'weighted_gain':round(gain,2),'bank_after':round(opt['bank']/10,1),'free_transfers_after':opt['free_transfers'],'hit_points':opt.get('hit_points',0),'plan':[public_move(m) for m in plan]},
'recommendation':{'edge':round(gain,2),'transfers':[{'out':row(byid[o],TARGET),'in':row(byid[i],TARGET)} for o,i in first_pairs] if go else []},
'comparison':{'status':'GJØR DET' if go else 'BANK','out':row(first_out,TARGET) if first_out else None,'in':row(first_in,TARGET) if first_in else None,'changes':[{'out':row(byid[o],TARGET),'in':row(byid[i],TARGET)} for o,i in first_pairs],'current_xi':xi_rows(cur,outs=outs),'transfer_xi':xi_rows(aft,ins=ins)},
'lineup':xi_rows(aft if go else cur,ins=ins if go else set()),'bench':[row(p,TARGET) for p in (after if go else squad) if p['id'] not in {x['id'] for x in (aft if go else cur)['xi']}],'candidates':cands,'future':future}
OUT.write_text(json.dumps(data,ensure_ascii=False,indent=2),encoding='utf-8');print('Model v2.3 strategy feed complete')