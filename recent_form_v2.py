...
def blend_rates(base,season_form,recent_form,attack_share=.40,enabled=True):
 """Blend overlapping form signals without multiplying the same matches twice.

 Season-form already contains current-season evidence. Recent form is therefore
 added as a confidence-weighted deviation around 1.0 instead of multiplied into
 the season signal. This is especially important after only one or two matches.
 """
 out=dict(base);sm=n(season_form.get('multiplier'),1);rm=n(recent_form.get('multiplier'),1);rc=clamp(n(recent_form.get('confidence'),0),0,1)
 recent_weight=clamp(float(attack_share),0,.65)*rc if enabled else 0.0
 combined=1+(sm-1)+(rm-1)*recent_weight
 combined=clamp(combined,.82,1.22)
 out['goal90']*=combined;out['assist90']*=combined
 # Pass evidence metadata through for the core's attacking-rate shrinkage.
 out['recent_minutes']=n(recent_form.get('minutes'),0)
 out['recent_confidence']=rc
 out['form_multiplier_used']=combined
 return out,combined
