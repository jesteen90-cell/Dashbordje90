...
d['candidate_selection']={'version':'1.1-decision-ready','method':'bench + horizons + reliability + option value + timing + regret','selected_candidate_index':selected['candidate_index'] if selected else None,'selected_signature':selected['signature'] if selected else '','selected_label':selected['label'] if selected else 'Ingen kandidat','agrees_with_optimizer_first_move':bool(selected and selected['signature'] and selected['signature']==plan_sig),'automatic_go_requires_optimizer_agreement':False,'package_comparison_authoritative':True,'rows':rows}
