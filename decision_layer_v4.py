...
def premium_feedback():
 ...
 try:d=json.loads(PREMIUM_PARAMS.read_text());base.update(d);base['candidate_bias']=max(0,min(.20,n(base.get('candidate_bias'))));return base
 except:return base
...
def input_quality_adjustment(data):
 """Raise the transfer bar when fixture/market evidence is weak; never lower it below calibrated threshold."""
 sm=data.get('team_strength_model') or {};sources=set(sm.get('prior_sources') or []);market=bool((data.get('market_ensemble') or {}).get('active'))
 if 'fpl-bootstrap' in sources:strength_q=1.0;strength_label='FPL strength prior'
 elif 'table-shrunk' in sources:strength_q=.55;strength_label='svak tabellprior'
 else:strength_q=.25;strength_label='nøytral fallback'
 market_q=1.0 if market else .25
 quality=.65*strength_q+.35*market_q
 # Up to +0.24 xP threshold when both independent inputs are weak.
 penalty=round(max(0,min(.24,(1-quality)*.32)),2)
 return {'score':round(quality,2),'threshold_penalty':penalty,'strength_quality':strength_label,'market_active':market,'prior_sources':sorted(sources)}
...
def captain_comparison(lineup):
 rows=[]
 for p in lineup:
  xp=n(p.get('xp'));mins=n(p.get('expected_minutes'),90);avail=n(p.get('availability'),1);ceiling=max(xp,n(p.get('xp_high'),xp));score=xp*.70+ceiling*.20+(mins/90)*.06+avail*.04;rows.append({'id':p.get('id'),'name':p.get('name'),'team':p.get('team'),'xp':round(xp,2),'ceiling':round(ceiling,2),'expected_minutes':round(mins,0),'availability':round(avail,2),'score':round(score,3),'captain':bool(p.get('captain')),'vice':bool(p.get('vice')),'season_minutes':p.get('season_minutes'),'season_points':p.get('season_points'),'season_xg':p.get('season_xg'),'season_xa':p.get('season_xa'),'season_goals':p.get('season_goals')})
 return sorted(rows,key=lambda x:x['score'],reverse=True)[:5]
def select_squad_view(data,approved):
 cmp=data.get('comparison') or {};changes=cmp.get('changes') or [];raw=n((data.get('optimizer') or {}).get('weighted_gain'));target_xi=list(cmp.get('transfer_xi') if approved else cmp.get('current_xi') or []);base=list(data.get('lineup') or [])+list(data.get('bench') or []);rows={int(p.get('id')):dict(p) for p in base if p.get('id') is not None};base_is_transfer=bool(changes) and raw>1.0
 ...
 xi_ids={int(p.get('id')) for p in target_xi if p.get('id') is not None};bench=[p for pid,p in rows.items() if pid not in xi_ids]
 if len(target_xi)==11 and len(bench)==4:data['lineup']=target_xi;data['bench']=reorder_bench(bench)
 else:data.setdefault('decision_layer_warnings',[]).append(f'squad-view reconstruction failed: xi={len(target_xi)} bench={len(bench)}')
 return data.get('lineup') or target_xi
def explain_decision(data,approved,blockers,controls,effective_gain,premium_bias,premium,quality):
 ...
 if quality['threshold_penalty']>0:reasons.append(f"Datakvalitet {quality['score']:.2f} ({quality['strength_quality']}, market {'på' if quality['market_active'] else 'av'}) løfter BYTT-kravet med {quality['threshold_penalty']:.2f} xP.")
 ...
 reasons.append(f"Gratisbytte-fleksibilitet verdsettes til {FT_FLEX_VALUE:.2f} xP; robust BYTT-terskel er {controls['threshold']:.2f}.");reasons.extend(blockers[:2] if blockers else ['Ingen harde minutt- eller tilgjengelighetsblokker stopper trekket.']);dist=round(controls['threshold']-effective_gain,2);trigger='Anbefalingen er robust nok nå.' if approved else f'Trenger omtrent {max(0,dist):.2f} mer netto modellfordel, eller at en blokkering forsvinner, før BYTT godkjennes.';return {'decision':'BYTT' if approved else 'BANK','why':reasons[:6],'weighted_gain':round(effective_gain,2),'threshold':round(controls['threshold'],2),'distance_to_switch':max(0,dist),'switch_trigger':trigger,'horizon_3gw':round(short,2),'horizon_plan':round(horizon,2),'premium_structure_bias':premium_bias,'input_quality':quality}
def main():
 data=json.loads(PATH.read_text());controls,feedback=adaptive_controls();premium=premium_feedback();quality=input_quality_adjustment(data);controls['threshold']=round(min(1.69,controls['threshold']+quality['threshold_penalty']),2);feedback['input_quality']=quality;feedback['effective_threshold']=controls['threshold'];candidates=data.get('candidates') or []
 for c in candidates:
 ...
 lineup=select_squad_view(data,approved);data['captain_comparison']=captain_comparison(lineup);data['decision_explanation']=explain_decision(data,approved,blockers,controls,effective_gain,pb,premium,quality);data['decision_layer']={'version':'4.6-input-quality-threshold','approved_first_move':approved,'threshold':round(controls['threshold'],2),'base_threshold':BASE_DO_THRESHOLD,'controls':{k:round(v,3) for k,v in controls.items()},'feedback':feedback,'input_quality':quality,'premium_structure_feedback':premium,'explainability':True,'captain_comparison':True,'backtest_adaptive':True,'component_adaptive':True,'premium_structure_adaptive':True,'squad_view_consistent':True};PATH.write_text(json.dumps(data,ensure_ascii=False,indent=2))
